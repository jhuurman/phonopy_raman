      ...
      real*8 APRIM(3,3),eps(3),eps_imag(3),x(3)
      ...
      real*8 RR(:,:),DISP(:,:),EPSIL(:,:,:),EPSIL_imag(:,:,:),EPSIL0(3,3)
      allocatable :: Rold,u,name,iatom,names,na,na1,na2,ch1
      allocatable :: RR,DISP,EPSIL,EPSIL_imag,outname,KSTEP,NameAtom
      real*8 :: tmp,frequency_specific
      real*8,allocatable :: frequency(:)
      real*8,allocatable :: eps_real_mat(:,:,:), eps_imag_mat(:,:,:)
      complex*16, allocatable :: eps_complex(:,:,:)
      character*60 ::  char1, char2, char3, char4, char5, char6
      character*200 :: line1,line2,line3,line,filename
      logical :: existence
...
!         read(11,*) frequency_specific
!         close(11)
!       else
        write(*,*) "Please input the frequency (eV):" 
        read(*,*) frequency_specific
!         open(unit=11,file="input")
!         write(11,*)  frequency_specific
!         close(11)
!       end if
...
! C read RAMDISCAR from PHONON ver.5.11
      ...
      enddo
! C-------------------------------------------------------
      allocate(outname(KI),RR(3,KI),DISP(3,KI),EPSIL(3,3,KI),EPSIL_imag(3,3,KI))
! !--------------------
...
! C Name of folders where are OUTCARs
      write(filename,"(A,F4.2)") 'EPSILON_',frequency_specific
      open(10,file=filename,ERR=9)
! C      write(10,999)info(IDIR)
      write(10,'(A)')' '//text
      write(10,*)' '
      write(10,'(A)')  ' Dielectric constant tensors from the following files:'
! C      write(10,'(A)')' - zero atomic displacments "zero.xml"'
      write(10,'(A)')'  - one displaced atom (from position X.Y,Z)'//                         ' along x, y, or z direction,'
      write(10,'(A)')'     with amplitude in angstroms.'
      write(10,'(A)') ' Atom position in fractional coordinates with respect to metrix:'
! ! Write metrix
      do J=1,3
...
! C Write displacement
       ...
       write(10,1003)outname(J), (Rold(iatom(J),JJ),JJ=1,3),(vv(JJ),JJ=1,3)
        do ji=1,3
...
! C------------------------------------------
       ...
       call readDiel(iuni,out,nHF,inx,IDIR,frequency_specific)
       if(inx.eq.1) goto 9
! C-----
       rewind(iuni)
       do I=1,3
         read(iuni,*)(eps(JJ),JJ=1,3)
         read(iuni,*)(eps_imag(JJ),JJ=1,3)
         write(10,"(2F13.4,A,2F13.4,A,2F13.4)") eps(1), eps_imag(1), "        ", eps(2), eps_imag(2),"        ",eps(3), eps_imag(3)
!          write(10,1002)(eps(JJ),JJ=1,3)
         do JJ=1,3
           EPSIL(I,JJ,J)=eps(JJ)
           EPSIL_imag(I,JJ,J)=eps_imag(JJ)
         enddo
       enddo
       close(iuni)
      enddo
...
! C Writes Raman matrices 3x3, and composses Raman Tensor 3x9
      call Raman_Tensor(APRIM,KI,outname,KSTEP,NameAtom,kki,RR,DISP,EPSIL,EPSIL_imag,EPSIL0,IDIR,info,text,frequency_specific)
! C------------------------------------------------------------------------
      deallocate(outname,RR,DISP,EPSIL,EPSIL_imag)
...
! C===========================================================
! C===========================================================
      SUBROUTINE readDiel(iuni,out,nHF,inx,IDIR,frequency_specific)
! C Subroutine cuts out HF forces from out=/ph_pos_Aa/OUTCAR
      IMPLICIT REAL*8 (A-H,O-Z)
      real*8 eps(3),eps_imag(3)
      ...
      real*8,allocatable :: eps_real_mat(:,:,:), eps_imag_mat(:,:,:)
      complex*16, allocatable :: eps_complex(:,:,:)
      integer :: n_frequency,index_frequency
      real*8,allocatable :: frequency(:)
      character*200 :: line1,line2,line3,line
      character*60 ::  char1, char2, char3, char4, char5, char6
...
! !       Find microscopic dielelectric constant
  31    read(15,1001,END=11)str
        if(str(2:37).NE.'MICROSCOPIC STATIC DIELECTRIC TENSOR') GOTO 31
...
!       Find macroscopic dielectric constant
  61    read(15,1001,END=51)str
        if(str(2:37).NE.'MACROSCOPIC STATIC DIELECTRIC TENSOR') GOTO 61
!       Find second dielectric constant
  62    read(15,1001,END=51)str
        if(str(2:37).NE.'MACROSCOPIC STATIC DIELECTRIC TENSOR') GOTO 62
   ...
   51   close(15)
! C-------------------
! File "vasprun.xml", frequency_dependent complex DIELECTRIC CONSTANT
        case(4)
        open(15,file=out,status='old',err=30)
...
!           line=trim(line)
!           line=adjustl(line)
!           read(line,*,end=3000) char1,char2,char3
!           if(char1=="<i" .and. char2=='type="int"' .and. char3=='name="NEDOS">') then
!             exit
!           end if
!         end do
!     
!     
!         read(line,*) char1,char2,char3,char1
!         read(char1(1:4),*) n_frequency
...
4000 do 
  ...
  line=trim(line)
  line=adjustl(line)
  read(line,*,end=4000) char1
  if(char1=="<dielectricfunction>" .or. char1=="<dielectricfunction") then
    exit
  end if
end do
...
do
  read(15,*)  char1  
  if(char1 == "<r>") then
    n_frequency=n_frequency+1
  else
    exit
  end if
end do
 
allocate(frequency(n_frequency), eps_complex(n_frequency,3,3))
allocate(eps_real_mat(n_frequency,3,3), eps_imag_mat(n_frequency,3,3)) 

do i=1,n_frequency+1
  backspace(15)
end do

!write(*,*) len(trim(adjustl(line)))

do i=1,n_frequency
  if(len(trim(adjustl(line))) < 90) then
    read(15,*)  char1, frequency(i), eps_imag_mat(i,1,1),eps_imag_mat(i,2,2),eps_imag_mat(i,3,3), eps_imag_mat(i,1,2),eps_imag_mat(i,2,3),eps_imag_mat(i,3,1)
    eps_imag_mat(i,2,1)=eps_imag_mat(i,1,2)
    eps_imag_mat(i,3,2)=eps_imag_mat(i,2,3)
    eps_imag_mat(i,1,3)=eps_imag_mat(i,3,1)
  else
    read(15,*)  char1, frequency(i), eps_imag_mat(i,1,1),eps_imag_mat(i,2,2),eps_imag_mat(i,3,3), eps_imag_mat(i,1,2),eps_imag_mat(i,2,3),eps_imag_mat(i,3,1), eps_imag_mat(i,2,1),eps_imag_mat(i,3,2),eps_imag_mat(i,1,3)
  end if
end do
...
do i=1,n_frequency
  if(len(trim(adjustl(line))) < 90) then
    read(15,*)  char1, frequency(i), eps_real_mat(i,1,1),eps_real_mat(i,2,2),eps_real_mat(i,3,3), eps_real_mat(i,1,2),eps_real_mat(i,2,3),eps_real_mat(i,3,1)
    eps_real_mat(i,2,1)=eps_real_mat(i,1,2)
    eps_real_mat(i,3,2)=eps_real_mat(i,2,3)
    eps_real_mat(i,1,3)=eps_real_mat(i,3,1)
  else
    read(15,*)  char1, frequency(i), eps_real_mat(i,1,1),eps_real_mat(i,2,2),eps_real_mat(i,3,3), eps_real_mat(i,1,2),eps_real_mat(i,2,3),eps_real_mat(i,3,1), eps_real_mat(i,2,1),eps_real_mat(i,3,2),eps_real_mat(i,1,3)
  end if
end do


        do i=1,n_frequency
          eps_complex(i,:,:)=cmplx(eps_real_mat(i,:,:),eps_imag_mat(i,:,:))
        end do
    
    
        tmp=1000000.0
        do i=1,n_frequency
          if(abs(frequency(i)-frequency_specific)<tmp) then
            tmp=abs(frequency(i)-frequency_specific)
            index_frequency=i
          end if
        end do
        
        do i=1,3
           eps(:)     = real(eps_complex(index_frequency,i,:))
           eps_imag(:)=aimag(eps_complex(index_frequency,i,:))
         write(iuni,*) eps
         write(iuni,*) eps_imag
        enddo
        
        deallocate(eps_complex)
        deallocate(eps_real_mat, eps_imag_mat)
!         do 
...
! C===========================================================
! C===========================================================
      SUBROUTINE Raman_Tensor(APRIM,KI,outname,KSTEP,NameAtom,kki,RR,DISP,EPSIL,EPSIL_imag,EPSIL0,IDIR,info,text,frequency_specific)
! C-----------------------------------------------------------
      real*8 APRIM(3,3),pi,pi4,VPRIM
      character*5 outname(KI)
      character*4 NameAtom(kki)
      character*60 text,filename
      character*74 info(4)
      real*8 :: tmp,frequency_specific
      integer i,II,j,k,l,KK,n,m,i1,i2,kkk,KSTEP(kki),IDIR
      integer low(:),high(:)
      real*8 RR(3,KI),DISP(3,KI),EPSIL(3,3,KI),EPSIL_imag(3,3,KI),EPSIL0(3,3)
!     real*8 EPSI0(3,3)
      real*8 EPSI(:,:,:),EPSI_imag(:,:,:),RRR(:,:),RAM(:,:,:),RAM_imag(:,:,:)
      real*8 A(3,3),AI(3,3),B(3),BB(3,3),BB_imag(3,3),C(3)  ! for matrix equation A.B=C, B - unknown
      allocatable :: EPSI,EPSI_imag,RRR,RAM,RAM_imag,low,high
! C-----------------------------------------------------------
      CALL DETER3(APRIM,VPRIM)
      allocate(EPSI(3,3,KI),EPSI_imag(3,3,KI))
! C Deviding dielectric constant by 4pi:
      ...
      do j=1,3
        ...
        EPSI_imag(i,j,k)=VPRIM*EPSIL_imag(i,j,k)/pi4
      enddo
      enddo
      enddo
! Open RAMFILE file
      write(filename,"(A,F4.2)") 'RAMFILE_',frequency_specific
...
! C For Raman tensor RAM=0.0
      allocate(RRR(3,kki),RAM(3,3*3,kki),RAM_imag(3,3*3,kki))
      do KK=1,kki
      do m=1,3*3
      do n=1,3
         RAM(n,m,KK)=0.0D00
         RAM_imag(n,m,KK)=0.0D00
      enddo
      enddo
      enddo
! C---------------------------------------------------------------------
...
! ! Raman tensor
       do II=low(KK),high(KK)-1,2     !  loop over II
          ...
          do n=1,3
          do m=1,3
            BB(n,m)=(EPSI(n,m,II)-EPSI(n,m,II+1))/(DISP(kr,II)-DISP(kr,II+1))
            BB_imag(n,m)=(EPSI_imag(n,m,II)-EPSI_imag(n,m,II+1))/(DISP(kr,II)-DISP(kr,II+1))
          enddo
          enddo
      select case(kr)
        case(1) 
!           write(10,228)(BB(1,m),m=1,3)
          write(10,"(A,2F13.4,A,2F13.4,A,2F13.4)") '! Raman Tensor X: ', BB(1,1), BB_imag(1,1), "        ",BB(1,2), BB_imag(1,2),"        ",BB(1,3), BB_imag(1,3)
        case(2) 
!           write(10,229)(BB(1,m),m=1,3)
          write(10,"(A,2F13.4,A,2F13.4,A,2F13.4)") '! Raman Tensor Y: ', BB(1,1), BB_imag(1,1), "        ",BB(1,2), BB_imag(1,2),"        ",BB(1,3), BB_imag(1,3)
        case(3) 
!           write(10,230)(BB(1,m),m=1,3)
          write(10,"(A,2F13.4,A,2F13.4,A,2F13.4)") '! Raman Tensor Z: ', BB(1,1), BB_imag(1,1), "        ",BB(1,2), BB_imag(1,2),"        ",BB(1,3), BB_imag(1,3)
      end select
!           write(10,231)(BB(2,m),m=1,3)
          write(10,"(A,2F13.4,A,2F13.4,A,2F13.4)") '!                 ', BB(2,1), BB_imag(2,1), "        ",BB(2,2), BB_imag(2,2),"        ",BB(2,3), BB_imag(2,3)
!           write(10,231)(BB(3,m),m=1,3)
          write(10,"(A,2F13.4,A,2F13.4,A,2F13.4)") '!                 ', BB(3,1), BB_imag(3,1), "        ",BB(3,2), BB_imag(3,2),"        ",BB(3,3), BB_imag(3,3)
!           write(10,207)
          write(10,"(A)")   '!                 '
  ...
  231 format('!                 ',3F20.7)
          do m=1,3
            ...
            RAM_imag(1,3*(kr-1)+m,KK)=BB_imag(1,m)
            RAM(2,3*(kr-1)+m,KK)=BB(2,m)
            RAM_imag(2,3*(kr-1)+m,KK)=BB_imag(2,m)
            RAM(3,3*(kr-1)+m,KK)=BB(3,m)
            RAM_imag(3,3*(kr-1)+m,KK)=BB_imag(3,m)
          enddo
       enddo    ! loop II
      enddo     ! loop KK
  237 format('!======================================================')
  ...
  236 format('! The Raman tensors are multiplied by volume of ','primitive unit cell Vprim, and devided by 4pi.')
  232 format('! Name of Atom with Displacements, Atomic ', 'Position (X,Y,Z), and Raman tensor:')
  ...
  234 format(1X,3F16.4,1X,3F16.4,1X,3F16.4)
      ...
      write(10,*) 
      write(10,*) "! The Imaginary Part of Raman tensor:"
      
      do KK=1,kki
        write(10,233)NameAtom(KK),(RRR(j,KK),j=1,3)
        khl=3
        do m=1,3
           write(10,234)(RAM_imag(m,j,KK),j=1,3*khl)
        enddo
      enddo     
      
      deallocate(low,high)
      deallocate(RRR,RAM,RAM_imag)
      close(10)
      deallocate(EPSI,EPSI_imag)
      END
...
! C-----------------------------------------------------------
      IMPLICIT REAL*8 (A-H,O-Z)
      REAL*8 A(3,3)
      V=A(1,1)*A(2,2)*A(3,3)+A(1,2)*A(2,3)*A(3,1)+A(1,3)*A(2,1)*A(3,2)-A(3,1)*A(2,2)*A(1,3)-A(3,2)*A(2,3)*A(1,1)-A(3,3)*A(2,1)*A(1,2)
      END
! C===========================================================
! c********************** END OF PROGRAM**********************
! C===========================================================
