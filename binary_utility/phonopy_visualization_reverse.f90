...
integer :: i,j,k,m,n,i_reverse_mode
...
write(*,*) "Please indicate which mode you want to reverse the direction"
read(*,*) i_reverse_mode
...
!   end if
  ...
  if(i == i_reverse_mode) then
    l_cylinder=4.0*factor*(-1.0)
    l_cone=1.5*factor*(-1.0)
  else
