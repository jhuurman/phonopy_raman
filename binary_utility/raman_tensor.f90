program raman_tensor_phonopy_complex
implicit none

! gfortran -ffree-line-length-none raman_tensor.f90 lib/lapack.a lib/blas.a -o raman_tensor

integer :: i,j,k,m,n,n_atom_layer,n_layer,n_band,n_tensor
integer :: n_type,total_atom,n_mode,n_atom_ineq
integer,allocatable :: n_atom(:),n_atom_super(:),index_ineq(:),index_all_ineq(:)
integer,allocatable :: n_atom_eq(:), index_all(:), index_mapping(:,:)
real*8 :: a_x,a_y,a_z
real*8 :: b_x,b_y,b_z
real*8 :: c_x,c_y,c_z
integer :: reason
integer,allocatable :: atomic_number(:),layer_atom_index(:,:)
...
real*8,allocatable :: M_mapping(:,:,:,:),M_mapping_inv(:,:,:,:)
real*8, allocatable :: reduced_raman_average(:),reduced_raman_average_full(:)
real*8 :: polarization_incident(3), polarization_scattered(3),intensity_tmp(3), M_tmp(3,3)
...
character*2 ::  at_type(100)=" ", band_index(100)=" "
...
integer :: option, flag
logical :: symbol_line,too_large,existence, symmetry
...
inquire(file="cutoff",exist=existence)
if(existence) then
  frequency_cutoff=1000000000.0
  write(*,*) "Note that frequency_cutoff is set at", frequency_cutoff
else
...
!!determine the mass for each atom from the chemical symbols in CONTCAR, but this may not work for isotope
...
!!determine the mass for each atom directy from Phonopy band.yaml

do    
  read(2,"(A)") line  
  line=trim(adjustl(line))    
  read(line,*,end=301) char_1
  301 if(char_1 .eq. "points:") then    
   exit    
  end if    
end do   
...
do    
  ...
  102 if(char_2 .eq. "The") then    
   exit    
  end if    
end do  

! write(*,"(A,I0)") "Number of lines is ", n_tensor
n_tensor=n_tensor/4

if(n_tensor == total_atom) then
  write(*,"(A)") "No symmetry is turned on, so in DFT calculations epsilon derivatives are calculated for every atom."
  symmetry = .false.
else
  write(*,"(A)") "Symmetry is turned on, so in DFT calculations epsilon derivatives are only calculated for symmetry independent atoms."
  symmetry = .true.
end if
...
do    
  read(3,"(A)") line    
  line=trim(adjustl(line))    
  read(line,*,end=103) char_1,char_2    
  103 if(char_2 .eq. "Name") then    
   exit    
  end if    
end do

if(symmetry) then
!!!!!!!! when symmetry is turned on
  open(unit=4,file="symmetry_operation_matrices")
  read(4,*) char_1, n_atom_ineq

  allocate(index_ineq(n_atom_ineq),n_atom_eq(n_atom_ineq))
  allocate(index_mapping(total_atom, total_atom))
  allocate(M_mapping(n_atom_ineq,total_atom,3,3),M_mapping_inv(n_atom_ineq,total_atom,3,3))

  read(4,*) char_1, index_ineq(:)

  do i=1,n_atom_ineq
    read(4,*) char_1, char_1, char_1, n_atom_eq(i)
    read(4,*) char_1, index_mapping(i,1:n_atom_eq(i)) 
  end do
  ...
  do i=1,n_atom_ineq
    do j=1,n_atom_eq(i)
      read(4,*) 
      do m=1,3
        read(4,*) M_mapping(i,j,m,:)
      end do 
      call matrix_inverse(M_mapping(i,j,:,:),3,M_mapping_inv(i,j,:,:))
    end do
  end do
  ...
  do i=1,n_atom_ineq
    read(3,*) 
    read(3,*) eps_der(index_ineq(i),1,1,:),eps_der(index_ineq(i),2,1,:),eps_der(index_ineq(i),3,1,:)
    read(3,*) eps_der(index_ineq(i),1,2,:),eps_der(index_ineq(i),2,2,:),eps_der(index_ineq(i),3,2,:)
    read(3,*) eps_der(index_ineq(i),1,3,:),eps_der(index_ineq(i),2,3,:),eps_der(index_ineq(i),3,3,:)
  end do
  ...
  do i=1,n_atom_ineq
    read(3,*) 
    read(3,*) eps_der_imag(index_ineq(i),1,1,:),eps_der_imag(index_ineq(i),2,1,:),eps_der_imag(index_ineq(i),3,1,:)
    read(3,*) eps_der_imag(index_ineq(i),1,2,:),eps_der_imag(index_ineq(i),2,2,:),eps_der_imag(index_ineq(i),3,2,:)
    read(3,*) eps_der_imag(index_ineq(i),1,3,:),eps_der_imag(index_ineq(i),2,3,:),eps_der_imag(index_ineq(i),3,3,:)
  end do   

else
!!!!!!!! when symmetry is turned off
  do i=1,total_atom
    read(3,*) 
    read(3,*) eps_der(i,1,1,:),eps_der(i,2,1,:),eps_der(i,3,1,:)
    read(3,*) eps_der(i,1,2,:),eps_der(i,2,2,:),eps_der(i,3,2,:)
    read(3,*) eps_der(i,1,3,:),eps_der(i,2,3,:),eps_der(i,3,3,:)
  end do
  ...
  do i=1,total_atom
    read(3,*) 
    read(3,*) eps_der_imag(i,1,1,:),eps_der_imag(i,2,1,:),eps_der_imag(i,3,1,:)
    read(3,*) eps_der_imag(i,1,2,:),eps_der_imag(i,2,2,:),eps_der_imag(i,3,2,:)
    read(3,*) eps_der_imag(i,1,3,:),eps_der_imag(i,2,3,:),eps_der_imag(i,3,3,:)
  end do
end if

close(3)

!!! If symmetry is on, symmetry operation matrices are needed to map the DFT-calculated epsilon derivatives of symmetry independent atoms to epsilon derivatives of all atoms. 
...
!!! along the x direction
      call matrix_multiplication(M_mapping_inv(i,j,:,:),eps_der(index_ineq(i),1,:,:),3,M_tmp)
      call matrix_multiplication(M_tmp,M_mapping(i,j,:,:),3,eps_der(index_mapping(i,j),1,:,:))
      eps_der(index_mapping(i,j),1,:,:)=1.0D0/M_mapping(i,j,1,1)*eps_der(index_mapping(i,j),1,:,:)
!!! along the y direction      
      call matrix_multiplication(M_mapping_inv(i,j,:,:),eps_der(index_ineq(i),2,:,:),3,M_tmp)
      call matrix_multiplication(M_tmp,M_mapping(i,j,:,:),3,eps_der(index_mapping(i,j),2,:,:))
      eps_der(index_mapping(i,j),2,:,:)=1.0D0/M_mapping(i,j,2,2)*eps_der(index_mapping(i,j),2,:,:)
!!! along the z direction      
      call matrix_multiplication(M_mapping_inv(i,j,:,:),eps_der(index_ineq(i),3,:,:),3,M_tmp)
      call matrix_multiplication(M_tmp,M_mapping(i,j,:,:),3,eps_der(index_mapping(i,j),3,:,:))
      eps_der(index_mapping(i,j),3,:,:)=1.0D0/M_mapping(i,j,3,3)*eps_der(index_mapping(i,j),3,:,:)   
  
!!! the Imaginary part  
!!! along the x direction
      call matrix_multiplication(M_mapping_inv(i,j,:,:),eps_der_imag(index_ineq(i),1,:,:),3,M_tmp)
      call matrix_multiplication(M_tmp,M_mapping(i,j,:,:),3,eps_der_imag(index_mapping(i,j),1,:,:))
      eps_der_imag(index_mapping(i,j),1,:,:)=1.0D0/M_mapping(i,j,1,1)*eps_der_imag(index_mapping(i,j),1,:,:)
!!! along the y direction      
      call matrix_multiplication(M_mapping_inv(i,j,:,:),eps_der_imag(index_ineq(i),2,:,:),3,M_tmp)
      call matrix_multiplication(M_tmp,M_mapping(i,j,:,:),3,eps_der_imag(index_mapping(i,j),2,:,:))
      eps_der_imag(index_mapping(i,j),2,:,:)=1.0D0/M_mapping(i,j,2,2)*eps_der_imag(index_mapping(i,j),2,:,:)
!!! along the z direction      
      call matrix_multiplication(M_mapping_inv(i,j,:,:),eps_der_imag(index_ineq(i),3,:,:),3,M_tmp)
      call matrix_multiplication(M_tmp,M_mapping(i,j,:,:),3,eps_der_imag(index_mapping(i,j),3,:,:))
      eps_der_imag(index_mapping(i,j),3,:,:)=1.0D0/M_mapping(i,j,3,3)*eps_der_imag(index_mapping(i,j),3,:,:)       
    end do
  end do
 
  open(unit=11,file="epsilon_derivative") 
  write(11,*)  " ! The Real Part of Raman tensor:"
  do i=1,n_atom_ineq
    do j=1,n_atom_eq(i)
      k=index_mapping(i,j)
      write(11,"(A,I0,3F10.6)") "     Atom ", k, x(k), y(k), z(k)
      write(11,"(A,3F14.4,A,3F14.4,A,3F14.4)")  "   ", eps_der(k,1,1,:),"  ",eps_der(k,2,1,:)," ",eps_der(k,3,1,:)
      write(11,"(A,3F14.4,A,3F14.4,A,3F14.4)")  "   ", eps_der(k,1,2,:),"  ",eps_der(k,2,2,:)," ",eps_der(k,3,2,:)
      write(11,"(A,3F14.4,A,3F14.4,A,3F14.4)")  "   ", eps_der(k,1,3,:),"  ",eps_der(k,2,3,:)," ",eps_der(k,3,3,:) 
    end do
  end do
  write(11,*)
  write(11,*)  " ! The Imaginary Part of Raman tensor:"
  do i=1,n_atom_ineq
    do j=1,n_atom_eq(i)
      k=index_mapping(i,j)
      write(11,"(A,I0,3F10.6)") "     Atom ", k, x(k), y(k), z(k)
      write(11,"(A,3F14.4,A,3F14.4,A,3F14.4)")  "   ", eps_der_imag(k,1,1,:),"  ",eps_der_imag(k,2,1,:)," ",eps_der_imag(k,3,1,:)
      write(11,"(A,3F14.4,A,3F14.4,A,3F14.4)")  "   ", eps_der_imag(k,1,2,:),"  ",eps_der_imag(k,2,2,:)," ",eps_der_imag(k,3,2,:)
      write(11,"(A,3F14.4,A,3F14.4,A,3F14.4)")  "   ", eps_der_imag(k,1,3,:),"  ",eps_der_imag(k,2,3,:)," ",eps_der_imag(k,3,3,:) 
    end do
  end do 
end if
...
allocate(reduced_raman_average_full(n_mode))
...
else if(axis .eq. "no") then   !!! average all polarizations of the incident and scattered light for polycrystals
  do i=1,n_mode
    reduced_raman_average(i)=abs(raman_tensor_cmplx(i,1,1))**2+abs(raman_tensor_cmplx(i,1,2))**2+abs(raman_tensor_cmplx(i,1,3))**2+abs(raman_tensor_cmplx(i,2,1))**2+abs(raman_tensor_cmplx(i,2,2))**2+abs(raman_tensor_cmplx(i,2,3))**2+abs(raman_tensor_cmplx(i,3,1))**2+abs(raman_tensor_cmplx(i,3,2))**2+abs(raman_tensor_cmplx(i,3,3))**2
    if(frequency(i) < frequency_cutoff ) then  !!! frequency is in the unit of THz
      reduced_raman_average(i)=reduced_raman_average(i)  !! frequency is too small to be divided
    else
      reduced_raman_average(i)=reduced_raman_average(i)/frequency(i)*(1.0+occupation(i)) ! reduced Raman divided by the frequency and multiplied by (1+n), so more like cross section now
...
end program raman_tensor_phonopy_complex
...
subroutine matrix_inverse(A,n,Ainv)
implicit none
integer :: n, info 
